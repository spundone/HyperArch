...
- Wants Raycast-like themed search, optional frosted-glass blur when transparency is enabled, and a rounded-corners toggle in the shell.
...
- Layer components live under `os updates/`; `hyperwebster-update` pulls the layer from GitHub and supports `--force-migrations` for stuck ISO installs.
