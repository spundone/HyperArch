...
# (~/.local/state/hyperwebster/applied). Always fetches the tarball; skips rsync/cp
# when the extracted tree matches on-disk files (unless --force).
set -euo pipefail
...
read_local_migrations() {
  [ -f "$VERSION_FILE" ] || return 0
  sed -n 's/^migrations=//p' "$VERSION_FILE" | head -1
}

count_on_disk_migrations() {
  find "$DEST/hyperwebster-update/migrations" -maxdepth 1 -name '*.sh' 2>/dev/null | wc -l | tr -d ' '
}

# Guard against commit-only skip when layer-version was updated without a full sync.
layer_metadata_consistent() {
  local on_disk recorded
  on_disk=$(count_on_disk_migrations)
  [ "${on_disk:-0}" -gt 0 ] || return 1
  recorded=$(read_local_migrations)
  if [ -n "${recorded:-}" ] && [ "$recorded" != "$on_disk" ]; then
    return 1
  fi
  return 0
}
...
write_version_file() {
  local commit="$1"
  local fetched mig
  fetched=$(date -Is 2>/dev/null || date -u +%Y-%m-%dT%H:%M:%SZ)
...
layer_trees_equal() {
  local src="$1" dest="$2" changes
  [ -d "$dest" ] || return 1
  if command -v rsync >/dev/null 2>&1; then
    # Empty itemize output => identical trees. Treat rsync errors as "not equal".
    changes=$(rsync -a --delete --dry-run --itemize-changes "$src/" "$dest/" 2>/dev/null) || return 1
    [ -z "$(printf '%s\n' "$changes" | grep -v '^$')" ]
    return $?
  fi
...
if [ "$FORCE_PULL" != 1 ] && layer_trees_equal "$src" "$DEST"; then
  if ! layer_metadata_consistent; then
    warn "layer-version migrations count mismatch — syncing anyway"
  else
