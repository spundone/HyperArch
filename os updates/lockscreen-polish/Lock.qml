...
Scope {
    ...
    IpcHandler {
        ...
        // Gamepad unlock path (controller-desktop). Session lock surfaces do
        // not see virtual uinput keyboards, so feed PAM via IPC instead.
        function feed(text: string): void {
            if (lock.locked)
                pam.feedText(text);
        }

        function pending(digit: string): void {
            if (lock.locked)
                pam.setPendingDigit(parseInt(digit, 10) || 0);
        }

        function bufferLen(): int {
            return pam.bufferLen();
        }

        function backspace(): void {
            if (lock.locked)
                pam.feedBackspace(false);
        }

        function clear(): void {
            if (lock.locked)
                pam.feedBackspace(true);
        }
