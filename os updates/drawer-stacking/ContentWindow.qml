...
StyledWindow {
    ...
    // HyperWebster: promote to Overlay while interactive drawers are open so the
    // top dashboard is not buried under nspanels (Overlay) or other Top surfaces.
    WlrLayershell.layer: (visibilities.dashboard || visibilities.launcher || visibilities.session || visibilities.sidebar || (fsTransitionProg > 0 && contentItem.Config.general.showOverFullscreen)) ? WlrLayer.Overlay : WlrLayer.Top
